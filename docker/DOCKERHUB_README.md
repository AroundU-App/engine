...
This Docker image will run Engine as a single process. By default it uses a
...
```
docker run -it --rm \
    --mount type=volume,src=engine-data,dst=/data \
    -e SYNAPSE_SERVER_NAME=my.matrix.host \
    -e SYNAPSE_REPORT_STATS=yes \
    baroundu/engine:latest generate
```

For information on picking a suitable server name, see
https://github.com/AroundU-App/engine/blob/develop/INSTALL.md.

The above command will generate a `homeserver.yaml` in (typically)
`/var/lib/docker/volumes/engine-data/_data`. You should check this file, and
...
* `SYNAPSE_HTTP_PORT`: the port Engine should listen on for http traffic.
      Defaults to `8008`.
...
Once you have a valid configuration file, you can start engine as follows:

```
docker run -d --name engine \
    --mount type=volume,src=engine-data,dst=/data \
    -p 8008:8008 \
    baroundu/engine:latest
```

(assuming 8008 is the port Engine is configured to listen on for http traffic.)
...
* `SYNAPSE_CONFIG_PATH`: path to the config file. Defaults to
  `<SYNAPSE_CONFIG_DIR>/homeserver.yaml`.
* `SYNAPSE_WORKER`: module to execute, used when running Engine with workers.
   Defaults to `synapse.app.homeserver`, which is suitable for non-worker mode.
* `UID`, `GID`: the user and group id to run Engine as. Defaults to `991`, `991`.
* `TZ`: the [timezone](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones) the container will run with. Defaults to `UTC`.

For more complex setups (e.g. for workers) you can also pass your args directly to Engine using `run` mode. For example like this:

```
docker run -d --name engine \
    --mount type=volume,src=engine-data,dst=/data \
    -p 8008:8008 \
    baroundu/engine:latest run \
...
After engine is running, you may wish to create a user via `register_new_matrix_user`.

This requires a `registration_shared_secret` to be set in your config file. Engine
must be restarted to pick up this change.

You can then call the script:

```
docker exec -it engine register_new_matrix_user http://localhost:8008 -c /data/homeserver.yaml --help
```
...
to use a reverse proxy, or configure Engine to expose an HTTPS port.

For documentation on using a reverse proxy, see
https://github.com/AroundU-App/engine/blob/master/docs/reverse_proxy.md.

For more information on enabling TLS support in Engine itself, see
https://github.com/AroundU-App/engine/blob/develop/INSTALL.md#tls-certificates. Of
...
raised if you try to run Engine without a config file.
...
docker run -it --rm \
    ...
    -e SYNAPSE_SERVER_NAME=my.matrix.host \
    -e SYNAPSE_REPORT_STATS=yes \
    baroundu/engine:latest migrate_config
```
...
If you need to build the image from a Engine checkout, use the following `docker
 build` command from the repo's root:

```
docker build -t baroundu/engine:latest -f docker/Dockerfile .
```
...
You can read about jemalloc by reading the Engine 
[README](https://github.com/AroundU-App/engine/blob/HEAD/README.rst).
